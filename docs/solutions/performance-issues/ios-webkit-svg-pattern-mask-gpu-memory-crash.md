...
**How the cost scales.** Nothing in the table depends on trail length: at most 6 bands are attached at once (visibility window 4100 px plus two 1600 px lookahead margins, `ceil(8900/1600) = 6`, `static/js/journey.js:1434-1438`), so every GPU figure is bounded by 6 times the per-band cost, and the double-length trail is a Chromium smoke scenario rather than a probe variant. Per band, `tileImages()` at `ART_TILE = 1024` emits 6 columns (x from -2048 to 3300) by 2 or 3 rows, so about 12 to 18 `<image>` elements per tiled layer, twice that in a winter band (ink plus snow). Shrinking `ART_TILE` grows that count quadratically (1024 to 256 is 16x more elements) and the probe has never varied it; only the dirt pattern's `TILE` has a lever. The one remaining pattern buffer scales roughly with `TILE` squared times DPR squared (base 170 MB at 256 vs 225 MB at 512, not a clean 4x, so treat it as approximate). All numbers are at the iPhone 13 descriptor's 3x DPR (`tests/smoke/webkit-gpu-memory.mjs:33`); a 2x device costs less, a larger 3x screen more.
...
- **Run the WebKit GPU probe before every staging round that touches band content.** Per the README's Development section: "`tests/smoke/webkit-gpu-memory.mjs` measures the GPU-process memory of Playwright's WebKit (Safari's engine) for URL variants at iPhone size, the best desktop proxy for the phone's crash ceiling. Run it against a running fixture server; compare every variant with `?ground=flat`." (README.md:104-107). Command, from the probe's own header (`tests/smoke/webkit-gpu-memory.mjs:14-17`):
  ...
  Two probe caveats: it finds WebKit processes by the hardcoded substring `webkit-2359` (`tests/smoke/webkit-gpu-memory.mjs:25`), so after a Playwright upgrade a reading of 0 MB across every variant means the filter broke, not that the build is free; and it reports one run's peak at 150 ms sampling with no pass/fail threshold, so differences of a few MB (the table's `?look=0` at 172 vs base at 170) are noise, not signal.
...
- **Band-content checklist for anything drawn inside a per-band `<svg>` (`static/js/journey.js`'s band-build loop, roughly lines 198-350):**
  ...
  - No element `opacity` on a band-sized or stroke shape in painted mode — use `stroke-opacity`/`fill-opacity` instead (`static/js/journey.js:460` and the comment at `static/js/journey.js:490`). The flat-mode fallback stroke at `static/js/journey.js:446` still uses element `opacity`; it predates this fix and is part of the 116 MB baseline, so it is excluded from this rule rather than evidence against it.
...
- **Every new per-band primitive gets its own off lever and a probe variant.** `?snow=off`, `?ink=off`, `?dirt=off` and `?sprites=off` are how the per-layer deltas in the table were measured; when U5 to U9 add water, sprites or a backdrop inside bands, add the matching `?x=off` lever first, then probe base against it.
