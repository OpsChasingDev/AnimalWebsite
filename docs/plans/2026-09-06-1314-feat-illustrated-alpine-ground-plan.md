...
**Deferred to Robert's staging review**

- The first-screen success criterion names the creek, but the creek is placed at the midpoint of the widest gap between events and U6 keeps that placement. Options: move the creek near the trailhead, or reword the criterion to "within the first stretch of trail". Default until Robert decides: keep the placement and judge the criterion on the first stretch.
...
- **Patterns to follow:** The seeded `rnd()` idea from `journey.js`, ported to Python's `random.Random(seed)`. There is no existing script in the repo to mirror.
- **Test scenarios:**
...
- **Files:** `app.py` (fixture hook in `get_model()`), `static/js/journey.js` (the `window.__journeyStats` hook inside `update()`), `tests/fixtures/journey-sample.json` (new, a redacted model with three pets, twelve camps, one creek, one pond, a lantern), `tests/smoke/journey-smoke.mjs` (new), `tests/smoke/package.json` (new, Playwright pinned), `README.md` (a Development paragraph including the browser install step).
- **Approach:**
  ...
  2. Add the `window.__journeyStats` hook inside `update()` now, filled with alive band count, alive 3D surfaces inside `#map`, bands missing an attached overlay while visible (zero whenever the page is in the flat state), the wall-clock duration of each `update()` call, and camera position, so the pre-art baseline uses the same hook. The smoke script starts the page with `?t=day`, scrolls the full trail in fixed steps and in three instant teleports, and records long frames through a `PerformanceObserver` for `longtask`. Frame time means the duration of each `update()` call as reported by the stats hook, not the interval between animation frames, which is pinned to the display refresh.
  3. Run three profiles: desktop; lite (coarse pointer emulation plus an iPhone user agent); desktop with `prefers-reduced-motion: reduce`. Assert per profile: zero console errors, alive bands never above 4 (the current cull window keeps four alive at about half of all camera positions) and attached overlays never above 5 with the lookahead, no element inside `#map` with a running animation under reduced motion, zero long tasks of 50 ms or more, and frame times within the Verification Contract ceilings. Record the current vector build's numbers as the baseline the ceilings must not regress past.
...
- **Test scenarios:**
  ...
  - The smoke script fails when an element inside `#map` has a running animation under the reduced-motion profile. The existing star twinkle outside `#map` does not trip it.
  - Covers AE2. With a fixture whose trail is twice the sample length, alive bands still never exceed 4.
...
- **Approach:**
  ...
  4. Replace the procedural scree, flowers and logs in the band-detail string with atlas cells, keeping their positions and the season rules that place them. An SVG image element cannot crop to a cell, so each detail sprite is a nested `svg` sized to the cell with a `viewBox` set to the cell's rectangle and one `image` of the whole atlas inside, so the atlas is one URL decoded once. U9 uses the same idiom through `prop()`. Remove what has no atlas cell.
...
- **Test scenarios:**
  ...
  - The smoke script fails when a band is left visible with no overlay for more than one second, simulated by blocking the overlay URL.
  ...
  - A trail twice the fixture length still produces at most 4 alive bands and at most 5 attached overlays.
...
- **Approach:**
  ...
  2. In `update()`, next to the existing ridge write, translate the backdrop by a fraction of `camY` (start near 0.03, tune on device) and by `camX` as today. Derive the vertical factor at load as the backdrop's spare height divided by the trail length, capped at 0.03, so the drift runs out exactly at the trail's end and never wraps (KTD2). A mountain range against sky does not tile vertically, so a loop would snap.
...
- **Test scenarios:**
  ...
  - Over a trail twice the fixture length, the backdrop's vertical offset is monotonic and never exceeds its spare height.
...
| Browser smoke | `cd tests/smoke && npm install && npx playwright install chromium && node journey-smoke.mjs` | U3 onward, all three profiles | Zero console errors; alive bands at most 4 and attached overlays at most 5; no blank band beyond 1 s; no running animation inside `#map` under reduced motion; zero long tasks of 50 ms or more; frame time is the duration of each `update()` call, desktop average at most 12 ms and worst 16 ms, lite average at most 14 ms and worst 20 ms; reduced motion matches its base profile; no regression past the U3 baseline |
